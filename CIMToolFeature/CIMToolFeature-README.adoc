...
or packaging workflow.* It is retained in the repository in the event that
feature-based deployment is ever resurrected. See <<status>> for details.
====
...
* The plugin version entries in `feature.xml` use `0.0.0` wildcards rather than
  pinned versions, to indicate the file is not being actively maintained
...
NOTE: `build.properties` references a `ChangeLog.txt` that no longer exists in
the project.
...
. Set `useFeatures="true"` in `CIMToolProduct/CIMTool.product`
. Update all plugin version references in `feature.xml` from `0.0.0` wildcards
  to the current release version
. Restore or recreate `ChangeLog.txt` referenced in `build.properties`
. Update the update site URLs in `feature.xml` to the current CIMTool
  distribution host
. Set up a p2 repository target alongside the PDE product export
