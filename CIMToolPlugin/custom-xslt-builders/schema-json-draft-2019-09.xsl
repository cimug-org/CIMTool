...
<xsl:stylesheet version="1.0"
	...
	<xsl:param name="schema_draft_version">https://json-schema.org/draft/2019-09/schema</xsl:param>
		...
		<xsl:variable name="super" select="a:SuperType" />	
	 	<xsl:choose>
			<xsl:when test="$super">
				...
				<xsl:if test="string-length($required_properties) > 0">
			...
			<xsl:otherwise>
				<xsl:if test="count(a:Complex|a:Enumerated|a:SimpleEnumerated|a:Simple|a:Domain|a:Instance|a:Reference|a:Choice) > 1">	
					<list begin="&quot;required&quot;: [" indent="    " delim="," end="]">	
						<xsl:for-each select="a:Complex|a:Enumerated|a:SimpleEnumerated|a:Simple|a:Domain|a:Instance|a:Reference|a:Choice">
							<xsl:if test="@minOccurs &gt;= 1">
								<item>"<xsl:value-of select="@name" />"</item>
			...
			<list begin="{{" indent="     " delim="," end="}}">
				...
				<xsl:if test="a:Root">
					<list begin="&quot;properties&quot;: {{" indent="    " delim="," end="}}">
						...
						<!-- DUMMY PLACEHOLDER TO ENSURE ALL JAVA CLASSES ARE GENERATED.  SIMPLE CREATES AN INLINE CLASS THAT CAN LATER BE REMOVED... -->
