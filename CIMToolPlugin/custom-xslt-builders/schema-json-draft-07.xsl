...
<xsl:stylesheet version="1.0"
	...
	<xsl:param name="generate_jsonschema2pojo_annotations">false</xsl:param>
	...
	*  01-15 (the leading - - are missing)
	...
	<xsl:template name="java_interface">
    	...
    	<xsl:choose>
    		<xsl:when test="$package">
    			<xsl:value-of select="concat($package_prefix, '.', translate($package, $uc, $lc), '.', 'I', $class_name)"/>
    		</xsl:when>
    		<xsl:otherwise>
    			<xsl:value-of select="concat($package_prefix, '.', 'I', $class_name)"/>
    		</xsl:otherwise>
    	</xsl:choose>
	</xsl:template>
			...
			<list begin="{{" indent="     " delim="," end="}}">
				...
				<xsl:if test="a:Root">
					<list begin="&quot;properties&quot;: {{" indent="    " delim="," end="}}">
						...
						<xsl:if test="$generate_jsonschema2pojo_annotations = 'true'">
							<!-- START: DUMMY PLACEHOLDER -->
							<!-- DUMMY PLACEHOLDER TO ENSURE ALL JAVA CLASSES ARE GENERATED.  SIMPLEY CREATES AN INLINE CLASS THAT CAN LATER BE REMOVED... -->
							<list begin="&quot;dependencies&quot;: {{" indent="    " delim="," end="}}">
								<item>"type": "object"</item>
								<list begin="&quot;properties&quot;: {{" indent="    " delim="," end="}}">
				...
				<list begin="&quot;definitions&quot;: {{" indent="     " delim="," end="}}">
					...
					<xsl:for-each select=".//a:Reference">
	...
	<xsl:template match="a:SimpleType" mode="declare">
		...
		<list begin="&quot;{@name}&quot;: {{" indent="    " delim="," end="}}">
			...
			<!--  The "value" property is always required  -->
			<list begin="&quot;required&quot;: [" indent="    " delim="," end="]">
				<item>"value"</item>
			</list>
		</list>
	...
	<xsl:template name="type_definition_header">
		...
		<xsl:choose>
			<xsl:when test="self::a:Root">
				<item>"$id": "#<xsl:value-of select="substring-after(@baseClass, '#')" />"</item>
		...
		<xsl:if test="$generate_jsonschema2pojo_annotations = 'true'">
			...
			<xsl:variable name="super" select="a:SuperType" />
			<xsl:if test="$super">
				<list begin="&quot;javaInterfaces&quot;: [" indent="    " delim="," end="]">
					<item>"<xsl:call-template name="java_interface"><xsl:with-param name="super_name" select="$super/@name"/></xsl:call-template>"</item>
				</list>
			</xsl:if>
		</xsl:if>
		<item>"type": "<xsl:value-of select="$type" />"</item>
	...
	<xsl:template match="a:Reference">
		...
		<list begin="&quot;{@name}&quot;: {{" indent="    " delim="," end="}}">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<item>"description": "<xsl:call-template name="annotate" select="../node()[@name = $ref_name]"/>"</item>
				...
				<xsl:when test="@maxOccurs = 'unbounded' or @maxOccurs &gt; 1">
					...
					<list begin="&quot;items&quot;: {{" indent="    " delim="," end="}}">
						...
						<list begin="&quot;oneOf&quot;: [" indent="    " delim="," end="]">
							<xsl:for-each
								select="a:Complex|a:Enumerated|a:SimpleEnumerated|a:Simple|a:Domain|a:Instance|a:Reference|a:Choice">
								<list begin="{{" indent="    " delim="," end="}}">
									<xsl:choose>
										<xsl:when test="self::a:Reference">
											<item>"$ref": "#/definitions/<xsl:value-of select="concat(@type, 'Ref')" />"</item>
