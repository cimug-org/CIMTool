...
1. **`configureLogging()`** — Suppresses spurious UCanAccess/HSQLDB reserved-word
...
2. **`extractLoggingProperties()`** — On first run, extracts `logging.properties`
...
- **CIMToolProduct** — the product project that packages CIMToolPlugin into a runnable application. `CIMToolPlugin.java` references the product bundle (`au.com.langdale.cimtool.product`) by its symbolic name when extracting `logging.properties` at startup.
