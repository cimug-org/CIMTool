...
<xsl:stylesheet version="3.0" 
	...
	<xsl:template match="a:Catalog">
		<document>
			<list begin="" indent="" delim="" end="">
				<item>//==================================================================</item>
				<item>// Following is the "header" attributes defined for this asciidoc   </item>
				<item>//==================================================================</item>
				<item>:stem: latexmath</item>
				<item>//</item>
				<!-- TEMPORARILY COMMENTED - TARGET PASSING STYLES/THEMES AS XSLT PARAMETERS
				<item>//========== Style definitions for PDF output ==========</item>
				<item>//</item>
				<item>:pdf-themesdir: {docdir}/Documentation/Themes</item>
				<item>:pdf-theme: adoc-article-rdfs-theme.yml</item>
				<item>:stylesdir: {docdir}/Documentation/Styles</item>
				<item>:stylesheet: adoc-article-rdfs.css</item>
				<item>//</item>
				-->
				<item>//============ General document settings ============</item>
				<item>//</item>
				...
				<item></item>
				<item>// TOC must appear after the title of the doc (above)</item>
				<item>:toc-placement: preamble</item>
				<item>:toc-title: Table of Contents</item>
				<item>:toc:</item>
				<item>:toclevels: 6</item>
				<item></item>
				<item>== Overview</item>
				<item></item>
				...
				<item></item>
				<item>== Profile Specification</item>
				<item></item>
	...
	<xsl:template match="a:Instance|a:Reference|a:Enumerated|a:Compound|a:Domain">
		...
		<item>|<xsl:if test="$roles != ''">[<xsl:value-of select="$roles"/>]#</xsl:if><xsl:value-of select="@name"/><xsl:text> </xsl:text>[<xsl:value-of select="@minOccurs"/>..<xsl:choose><xsl:when test="@maxOccurs = 'unbounded'">*</xsl:when><xsl:otherwise><xsl:value-of select="@maxOccurs"/></xsl:otherwise></xsl:choose>]<xsl:if test="$roles != ''">#</xsl:if><xsl:text> </xsl:text><xsl:call-template name="process-attribute-stereotypes"/></item>
	...
	<xsl:template match="a:Simple">
		...
		<item>|<xsl:if test="$roles != ''">[<xsl:value-of select="$roles"/>]#</xsl:if><xsl:value-of select="@name"/><xsl:text> </xsl:text>[<xsl:value-of select="@minOccurs"/>..<xsl:value-of select="@maxOccurs"/>]<xsl:if test="$roles != ''">#</xsl:if><xsl:text> </xsl:text><xsl:call-template name="process-attribute-stereotypes"/></item>
