...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:a="http://langdale.com.au/2005/Message#" xmlns="http://langdale.com.au/2009/Indent">
	...
	<xsl:template match="a:Catalog">
		<document>
			<list begin="@startuml" indent="" delim="" end="@enduml">
				...
				<xsl:apply-templates select="a:Root|a:ComplexType|a:EnumeratedType|a:CompoundType"/>			
			</list>
		</document>
	</xsl:template>
	...
	<xsl:template match="a:CompoundType">
		<xsl:if test="not(@hideInDiagrams = 'true')">
			...
			<item>' <xsl:value-of select="$compoundName"/> compound class</item>
			<list begin="class {concat($compoundName, ' ', $stereotypes, ' ', '#orange', ' &#123;')}" indent="   " delim="" end="{concat('&#125;', '&#xD;', '&#xA;')}">
				<xsl:choose>
					<xsl:when test="not(a:Stereotype[substring-after(., '#') = 'diagramshideallattributes'])">
						<xsl:apply-templates select="a:Complex|a:Enumerated|a:SimpleEnumerated|a:Simple|a:Domain"/>
					</xsl:when>
					<xsl:otherwise>
		...
		<xsl:if test="not(@hideInDiagrams = 'true')">
			...
			<xsl:variable name="color">
				<xsl:choose>
					...
					<xsl:when test="a:Stereotype[@label='Description']">#lightyellow</xsl:when>
					<!-- light gray for classes that are abstract stereotype -->
					<xsl:when test="not(a:Stereotype[@label='Concrete'])">#lightgray</xsl:when>
	...
	<xsl:template match="a:Reference|a:Instance">
		<xsl:if test="not(@hideInDiagrams = 'true')">
			...
			<item><xsl:value-of select="$sourceClass"/> "<xsl:value-of select="concat($sourceRoleEndName, ' ', $sourceCardinality)"/>"<xsl:value-of select="concat(' ', $associationType, ' ')"/>"<xsl:value-of select="concat($targetRoleEndName, ' ', $targetCardinality)"/>" <xsl:value-of select="$targetClass"/> <xsl:if test="a:Stereotype[substring-after(., '#') = 'enumeration'] or a:Stereotype[substring-after(., '#') = 'compound'] or (self::a:Reference and not(a:Stereotype[substring-after(., '#') = 'byreference']))">#red</xsl:if></item>
			<xsl:if test="not(//a:ComplexType[@name = $targetClass]|//a:Root[@name = $targetClass]|//a:CompoundType[@name = $targetClass]|//a:EnumeratedTye[@name = $targetClass])">
				<item>&#xD;&#xA;</item> <!-- CR/LF -->
				<list begin="" indent="" delim="" end="">
					<item>' This absract indicates an "orphan" reference on an invalid Reference/Instance that must be fixed in the profile</item>
	...
	<xsl:template match="a:Simple">	
		<xsl:if test="not(@hideInDiagrams = 'true')">
			<item>+<xsl:value-of select="@name"/> : <xsl:value-of select="@xstype"/> [<xsl:call-template name="cardinality"/>]</item>
		</xsl:if>
	</xsl:template>
			...
			<xsl:variable name="stereotypes">
				<xsl:for-each select="a:Stereotype">
					...
					<xsl:choose>
						...
						<xsl:when test="not(($stereotype = 'diagramshideallattributes') or ($stereotype = 'diagramshide') or ($stereotype = 'attribute') or ($stereotype = 'byreference') or ($stereotype = 'enum') or ($stereotype = 'concrete') or ($stereotype = 'ofAggregate') or ($stereotype = 'aggregateOf') or ($stereotype = 'ofComposite') or ($stereotype = 'compositeOf'))">
