...
<xsl:stylesheet version="1.0"
    ...
    *  01-15 (the leading - - are missing)
    ...
    <xsl:template name="required_choice_properties">
        <xsl:choose>
            <xsl:when test="a:SuperType">
                <xsl:variable name="supertype_name" select="a:SuperType/@name" />
                <xsl:for-each select="/*/node()[@name = $supertype_name]">
                    <xsl:call-template name="required_choice_properties" />
                </xsl:for-each>
                <xsl:for-each select="a:Choice[@minOccurs >= 1]">
                    <xsl:choose>
	...
	<xsl:template name="count_required_choices">
        <xsl:param name="count" />
        <xsl:variable name="total_count" select="$count + count(a:Choice[@minOccurs >= 1])"/>
		<xsl:choose>
            <xsl:when test="a:SuperType">
                <xsl:variable name="supertype_name" select="a:SuperType/@name" />
                <xsl:for-each select="/*/node()[@name = $supertype_name]">
    ...
    <xsl:template name="type_definition_header">
        ...
        <xsl:choose>
            <xsl:when test="self::a:Root">
                <!-- REMOVING THIS FROM draft-07 mappings... <item>"$id": "#<xsl:value-of select="substring-after(@baseClass, '#')" />"</item> -->
                <item>"title": "<xsl:value-of select="substring-after(@baseClass, '#')" />"</item>
            </xsl:when>
            <xsl:otherwise>
                <!-- REMOVING THIS FROM draft-07 mappings... <item>"$id": "#<xsl:value-of select="@name" />"</item> -->
