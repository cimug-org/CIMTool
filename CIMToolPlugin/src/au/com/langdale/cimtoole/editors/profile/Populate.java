...
public class Populate extends FurnishedEditor {
	...
	public boolean confirm(String name, int many) {
		...
		return MessageDialog.openConfirm(shell, "CIMTool",
	...
	protected Content createContent() {
		...
		return new Content(master.getToolkit()) {
			...
			protected Template define() {
				return Form(Grid(
						Group(Stack(
								Span(Label("objectprop", "Add associated classes to the profile or"),
										PushButton("jumpa", "Goto Restriction Page"),
										Right(PushButton("anon-left", "Create a local, anonymous profile class",
												"anontype"))),
								Span(Label("dataprop", ""), PushButton("jumpb", "Goto Restriction Page")),
								Span(Label("class", "Select members of this class."),
										ViewCheckBox("showsuper", "Show superclass members"),
										ViewCheckBox("showsub", "Show subclass members")),
								Span(Label("top", "Select classes or packages to profile."),
						...
						Group(TreeViewer("left", true), TreeViewer("right", true)),
						Group(Row(
								Span(ViewCheckBox("concrete",
										"Set selected classes to concrete when added to the profile")),
								Span(ViewCheckBox("required",
										"Set selected attributes to required when added to the profile")))),
						Group(Row(
								Span(ViewCheckBox("use-schema-cardinality", "Set cardinality of selected attributes to that defined in the schema")),
								Span(ViewCheckBox("include-all-associations", "Add all source-side associations when added to the profile")))), 
						Group(
			...
			private Picker allLeft = new Picker() {
				protected void handle(Node node) {
					...
					if (args.size() < 50 || confirm(node.toString(), args.size())) {
						...
						target.profileAddAllDeep(args, selectionOptions);
			...
			private Picker anonLeft = new Picker() {
				protected void handle(Node node) {
					// Anon classes are never concrete so we ensure we 
					// filter out the Concrete selection option should 
					// the user select the checkbox.
					SelectionOptions selectionOptions = getSelectionOptions();
					selectionOptions = selectionOptions.cloneAndRemove(SelectionOption.Concrete);
					SortedNode target = getProfileNode();
					target.profileAddAnon(node, selectionOptions);
					resetSelectionButtonStates();
				}
			};
			...
			private Picker toLeft = new Picker() {
				protected void handle(Node node) {
					...
					if (args.size() < 50 || confirm(node.toString(), args.size())) {
						SelectionOptions selectionOptions = getSelectionOptions();
						target.profileAddAll(args, selectionOptions);
						resetSelectionButtonStates();
					}
				}
			};
			...
			public void refresh() {
				...
				if (node instanceof CatalogNode || node instanceof EnvelopeNode) {
					...
					if (enode.isDatatype()) {
						...
						setTextValue("dataprop", "The datatype " + TreeModelBase.label(enode.getBase().getRange())
