...
public class ImportArchive extends Wizard implements IImportWizard {
    ...
    private FurnishedWizardPage select = new FurnishedWizardPage("select",
	    "Import a Model", null) {
	{
	    setDescription("Import a Models (CIM/XML files) from a Zip Archive.");
	}
	...
	protected Content createContent() {
		...
		protected Template define() {
		    return 
		        Grid(
			    Group(FileField("source", "Archive file:", new String[] { "*.zip", "*.zipped" })),
			    Group(Grid(
			        Group(Label("Default Namespace:"), Field("namespace")))), 
			        Group(Label("Project")), 
	...
	protected Content createContent() {
		...
		protected Template define() {
		    return 
		        Grid(
		            Group(Label("Models"), Label("Profiles")), 
		...
		public String validate() {
		    ...
		    getLabel("size").setText(
			    "Size of source is " + Long.toString(length)
				    + " bytes.");
			...
			suggestion = path.removeFileExtension().lastSegment()
	...
	IWorkspaceRunnable op = new SplitModelImporter((IFolder) destin,
		pathname, namespace.getText(), profiles.getFile(), null);
