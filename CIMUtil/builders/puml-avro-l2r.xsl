...
-->
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:a="http://langdale.com.au/2005/Message#" xmlns="http://langdale.com.au/2009/Indent" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:cimtool="http://cimtool.ucaiug.io/functions">
	...
	<xsl:template match="a:Root|a:ComplexType">
		<xsl:if test="not(@hideInDiagrams = 'true')">
			...
			<xsl:variable name="error" select="if ((self::a:ComplexType and (($subCount = 0 and $superCount > 0 and $refsCount > 0 and $fieldsCount > 0) or ($subCount = 0 and $superCount = 0 and $refsCount > 0 and $fieldsCount > 0) or ($subCount = 0 and $superCount = 0 and $refsCount = 0) or ($subCount = 0 and $superCount > 0 and $refsCount = 0))) or (self::a:Root and ($subCount = 0 and $superCount = 0 and $refsCount > 0 and $fieldsCount = 0))) then true() else false()" />
			<!-- ================================================================================================================= -->
			<xsl:variable name="className" select="substring-after(@baseClass, '#')"/>
			<xsl:variable name="stereotypes"><xsl:call-template name="stereotypes"/></xsl:variable>
			<xsl:choose>
			...
			<xsl:if test="$error">
				...
				<xsl:choose>
					...
					<xsl:when test="self::a:ComplexType and ($subCount = 0 and $superCount > 0 and $refsCount > 0 and $fieldsCount > 0)">
						<item>Class <xsl:value-of select="$className"/> is 'abstract' and has no child classes. </item>
						<item>It should either be removed, declared as 'concrete', have a child</item>
						<item>class that is declared as 'concrete', or if it is intended to serve</item>
						<item>as a reference to an external object then it should not inherit from </item>
						<item>a parent class.</item>
					</xsl:when>
					<xsl:when test="self::a:ComplexType and ($subCount = 0 and $superCount = 0 and $refsCount > 0 and $fieldsCount > 0)">
