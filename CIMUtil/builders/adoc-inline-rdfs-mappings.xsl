		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:if test="not(child::a:Stereotype[contains(., '#concrete')]) and (count(/.//a:Reference[@baseClass=$baseClass]) > 0)">
					...
					<item>This abstract class is a placeholder for 'By Reference' associations defined within this profile and as such has no attributes or associations defined. Rather, it is used to reference a corresponding concrete class defined in an external profile that this one depends upon.</item>
					<item></item>
					<item>endif::<xsl:value-of select="$fileName"/>-byref-profile[]</item>					
				</xsl:if>
			</xsl:otherwise>
