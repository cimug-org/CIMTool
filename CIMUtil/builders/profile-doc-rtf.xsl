...
<xsl:stylesheet version="2.0" 
	...
	exclude-result-prefixes="xs cimtool"
...
<xsl:function name="cimtool:rtf-escape" as="xs:string">
    <xsl:param name="text" as="xs:string"/>

    <!-- Convert string → codepoints, then map each codepoint -->
    <xsl:sequence select="
        string-join(
          for $cp in string-to-codepoints($text)
          return
            if ($cp ge $ascii-min and $cp le $ascii-max)
    ...
    <xsl:variable name="after-hex">
        <xsl:analyze-string select="$text" regex="&amp;#x([0-9A-Fa-f]+);">
            <xsl:matching-substring>
                ...
                <xsl:value-of select="codepoints-to-string($cp)"/>
            </xsl:matching-substring>
...
<xsl:template match="a:Catalog">{\rtf1\ansi\deff0\uc1\deflang1033\deflangfe1033\fs20{\fonttbl{\f0\fswiss\fprq2\fcharset0 Arial;}{\f1\fswiss\fprq2\fcharset0{\*\fname Arial;}Arial CE;}
...
{\par\pard\plain \sb120\qj\fs20\lang1033 <xsl:value-of select="cimtool:rtf-escape(cimtool:unescape-numeric(string(.)))"/>}
</xsl:template>
...
{\sb120\sa120 <xsl:if test="preceding-sibling::a:Comment or preceding-sibling::a:Note">\line\line </xsl:if><xsl:value-of select="cimtool:rtf-escape(cimtool:unescape-numeric(string(.)))"/>}
</xsl:template>
...
<xsl:value-of select="cimtool:rtf-escape(cimtool:unescape-numeric(string(.)))"/><xsl:if test="following-sibling::a:Comment or following-sibling::a:Note">\par </xsl:if>
</xsl:template>
