...
@Rule(name = "Missing Attribute Documentation", description = "Detects attributes with missing documentation")
public class MissingAttributeCommentRule {
    @Condition
    public boolean when(@Fact("resource") OntResource res, @Fact("baseURI") String baseURI) {
        return res.hasProperty(UML.hasStereotype, UML.attribute)
    ...
    public void then(@Fact("resource") OntResource res, @Fact("issues") List<String> issues, @Fact("baseURI") String baseURI) {
        issues.add("Attribute <" + res.getURI() + "> is missing proper documentation.");
    }
}