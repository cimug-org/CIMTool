...
public class ConsistencyChecker {
	...
	/**
	 * Retrieve a model containing error annotations. For each inconsistent
	 * statement in the target, the log contains one or more LOG.hasProblems
	 * statements. The subjects of the problem is a resource in the target
	 * associated with the inconsistent statement.
	 * 
	...
	private Terms Terms(FrontsNode t1) {
		return new Terms(new FrontsNode[] { t1 });
	}

	private Terms Terms(FrontsNode t1, FrontsNode t2) {
		return new Terms(new FrontsNode[] { t1, t2 });
	}

	private Terms Terms(FrontsNode t1, FrontsNode t2, FrontsNode t3) {
		return new Terms(new FrontsNode[] { t1, t2, t3 });
	}

	/**
	 * Check a target model for inconsistent use of a namespace. Any resource in the
	 * given namespace that is used in the target must be defined in the reference.
	 * Errors are recorded in the log model.
	 * 
	...
	/**
	 * True if the resource belongs to the reference schema (as opposed to the
	 * profile).
	 */
	private boolean isRefNode(Resource s) {
		...
		return !(ns.equals(namespace) || ns.equals(OWL.NS) || ns.equals(RDFS.getURI()) || ns.equals(RDF.getURI())
				|| ns.equals(MESSAGE.NS) || ns.equals(BUILDLET_NS));
	}
	...
	private void checkObject(OntResource subj, Property pred, OntResource obj) {
		...
		else if (Terms(RDFS.subClassOf, RDFS.domain, RDF.type).includes(pred) && !Terms(OWL.Class).hasInstance(obj))
	...
	private void note(Resource refNode, String mesg, Resource targNode) {
		...
		prob.addProperty(LOG.problemDetail, refNode.getURI() + " " + mesg
				+ (targNode == null ? "" : ", is linked to profile " + targNode.getURI()));
