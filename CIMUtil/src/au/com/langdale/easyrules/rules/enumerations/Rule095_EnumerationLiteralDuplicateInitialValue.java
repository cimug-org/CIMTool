...
@Rule(name = "Rule095", description = "In instances where an initial value for enumeration literals is used, the code must be unique among all the codes for enumeration literals within the containing CIM class.")
@RuleMetadata(type = RuleType.Normative, category = RuleCategory.Enumeration, errorTemplate = "In instances where an initial value for enumeration literals is used, the code must be unique among all the codes for enumeration literals within the containing enumeration. The initial value of {elementTypeLowerCase} {name} should be corrected in the model.")
public class Rule095_EnumerationLiteralDuplicateInitialValue extends OntResourceBaseRule {

	public Rule095_EnumerationLiteralDuplicateInitialValue(String baseURI) {
		super(baseURI);
	}
