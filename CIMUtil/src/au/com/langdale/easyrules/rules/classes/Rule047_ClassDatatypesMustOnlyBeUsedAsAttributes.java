...
@Rule(name = "Rule047", description = "Datatype classes must only be used as the datatype for attributes.")
@RuleMetadata(type = RuleType.Normative, category = RuleCategory.Class, errorTemplate = "{elementType} {name} has a restricted stereotype ({stereotype}) and therefore must only be used as a datatype for attributes. The model should be reviewed as it currently participates in the following association(s):\n {fullyQualifiedAssociations}")
public class Rule047_ClassDatatypesMustOnlyBeUsedAsAttributes extends OntResourceBaseRule {

	public Rule047_ClassDatatypesMustOnlyBeUsedAsAttributes(String baseURI) {
		super(baseURI);
	}
