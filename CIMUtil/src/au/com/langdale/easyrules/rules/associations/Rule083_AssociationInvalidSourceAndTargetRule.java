...
@Rule(name = "Rule083", description = "Associations should be drawn from the dependent (source) class to the target class in Enterprise Architect to facilitate correct dependency processing.")
@RuleMetadata(type = RuleType.Normative, category = RuleCategory.Association, errorTemplate = "{elementType} {name} should be drawn from the dependent (source) class (i.e. {correctSourceClass}) to the target class (i.e. {correctTargetClass}) in Enterprise Architect to facilitate correct dependency processing. Source and target ends are currently swapped.")
public class Rule083_AssociationInvalidSourceAndTargetRule extends OntResourceBaseRule {

	public Rule083_AssociationInvalidSourceAndTargetRule(String baseURI) {
		super(baseURI);
	}
		...
		if (isAssociation(resource) && isNormative(resource)) {
			...
			if (domain != null && range != null) {
				if (CIMRuleUtils.isSourceSide(resource)) {
					...
					// Again, if it isn't we know that the source and target are not correct.
					if (CIMRuleUtils.getDependencyPriority(domain) < CIMRuleUtils.getDependencyPriority(range)) {
						return true;
					}
				}
