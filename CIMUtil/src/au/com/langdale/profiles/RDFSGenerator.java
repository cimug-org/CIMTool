...
/**
 * A <code>SchemaGenerator</code> for creating a <i>legacy RDFS</i> model from a
 * profile model.
 */

public class RDFSGenerator extends RDFSBasedGenerator {

	public RDFSGenerator(OntModel profileModel, OntModel backgroundModel, boolean preserveNamespaces,
	...
	@Override
	protected void emitObjectProperty(String uri, OntResource baseProp, String domain, String range,
		...
		/**
		 * If set any min or max cardinality restrictions that may exist. It is possible
		 * that none are specified and in that case the defaults are used.
		 */
	...
	private void emitCardinality(Resource prop, boolean required, boolean functional, int minCard, int maxCard) {
		Resource card;
		if (functional) {
			if (required)
				card = CIMS.M1;
	...
	@Override
	protected void emitDatatypeProperty(String uri, OntResource baseProp, String domain, String type, String xsdtype,
			boolean required) {
		Resource prop = emit(uri, RDF.Property);

		boolean isFunctional = baseProp.isFunctionalProperty();
		
		int minCard = 0; // Default min
		int maxCard = (isFunctional ? 1 : Integer.MAX_VALUE); // Default max
		...
		emitCardinality(prop, required, isFunctional, minCard, maxCard);
	...
	@Override
	protected void emitRestriction(String uri, String domain, boolean required, boolean functional, int minCard,
			int maxCard) {
		// ignored
	}
