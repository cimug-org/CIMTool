...
<!--
  ...
  Whitespace-preserving variant of indent.xsl applied as the post-processing
  stage for ASCIIDOC type builders. Text originating from a:AsciiDoc elements
  is marked xml:space="preserve" by ProfileSerializer.emitAsciiDoc() and the
  attribute is propagated onto the enclosing in:item elements by the AsciiDoc
  builders. Such text is emitted verbatim so that authored indentation (e.g.
  Python source listings in AsciiDoc documentation) survives into the generated
  artifacts. All other text is normalized exactly as in indent.xsl, whose
  behavior this stylesheet otherwise mirrors. The historical indent.xsl remains
  unchanged and continues to serve all TEXT type builders. See defect #288.
-->
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="text()">
        ...
        <xsl:value-of select="concat($prefix,$begin,$body,$end,$suffix)"/>
    </xsl:template>
