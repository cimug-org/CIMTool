...
`.feap` | Native EA project file. Theis EA project file format has existed for a while and is the only EA project file format that does not require conversion when opening in a 64-bit release of EA 16.x or later.  The internal format of these files is based on the [Firebird SQL](http://www.firebirdsql.org/) open source database and stored in binary form. | This native EA project file format is supported starting with release 2.1.0 of **CIMTool** and can be imported directly without the overhead of having to export an `.xmi` file.| | 
