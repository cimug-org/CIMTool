...
public abstract class AbstractReader extends XMLReaderBase {
	...
	/**
	 * Convenience class to generate an element from a given word or the current
	 * token.
	 */
	protected class Element {
		...
		public void commit() throws SAXException {
			if (!committed) {
				...
				if (spaces.processName(type, parts, false) == null)
					throw new SAXException("undeclared namespace prefix: "
							+ type);
		...
		public void set(String name, String value) throws SAXException {
			...
			if (spaces.processName(name, attr_parts, true) == null)
				throw new SAXException("undeclared namespace prefix in: "
						+ name);

			atts.addAttribute(attr_parts[0], attr_parts[1], attr_parts[2],
					"CDATA", value);
		}
	...
	protected void parse() throws SAXException, IOException {
        ...
        protected abstract void emit() throws SAXException;
}
