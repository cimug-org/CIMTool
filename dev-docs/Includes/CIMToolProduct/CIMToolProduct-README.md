...
5. Bundling `logging.properties` and `logback.xml` for extraction to the installation root at runtime, enabling the unified single-file logging architecture where all three logging subsystems (JUL, SLF4J-native, Log4j 1.x) converge into `logs/cimtool.log`
...
├── logging.properties          ← JUL pre-bridge config — handlers= empty, .level=ALL; Logback is the sole output target
├── logback.xml                 ← Logback config — SLF4J → Logback pipeline; extracted to install root at startup
├── logback-dev.xml             ← Development-only Logback config — intentionally excluded from bin.includes and not bundled in the product export
├── release-cimtool.bat    ← Post-export signing and packaging pipeline (Windows only)
...
`org.eclipse.m2e.logback` is intentionally excluded from the `<plugins>` list. This
m2e bundle registers a `LogbackConfigurator` OSGi service that re-configures Logback
after the initial startup sequence completes, replacing the FILE appender loaded from
`logback.xml` with its own console-only configuration. Including it causes
`logs/cimtool.log` to be created but remain permanently empty for the lifetime of the
JVM process.
...
| Logging config (JUL) | `-Djava.util.logging.config.file=./logging.properties` |
| Logging config (Logback) | `-Dlogback.configurationFile=./logback.xml` |
...
Configures `java.util.logging` (JUL) for pre-bridge filtering in the CIMTool
application. This file is:
...
Configures Logback for the CIMTool application. This file is:
...
UCanAccess/HSQLDB warnings at `ERROR` level, and sets the root logger to `WARN`
for production use.

> **Support diagnostics:** To capture detailed output when investigating a
> reported issue, change `<root level="WARN">` to `<root level="DEBUG">` or
> `<root level="TRACE">` in `logback.xml` and restart CIMTool. Restore to
> `WARN` when diagnostics are complete.

Changes to `logback.xml` take effect on the next restart.
...
└── CIMTool-<version>/
    ...
    ├── CIMTool.ini             ← JVM arguments (heap, logging config paths)
    ├── logging.properties      ← Extracted here by CIMToolPlugin on first run
    ├── logback.xml             ← Extracted here by CIMToolPlugin on first run
    ├── logs/                   ← Created by CIMToolPlugin on first run; contains cimtool.log
    ├── plugins/
...
> **Note:** Neither `logging.properties` nor `logback.xml` at the installation
> root are placed there by the PDE export directly. Both are extracted from the
...
The `release-cimtool.bat` script located in the `CIMToolProduct/` directory handles the complete post-export signing and packaging pipeline. It must be run from a standard Windows Command Prompt (not PowerShell) after the PDE product export completes.

**Usage:**
```cmd
release-cimtool.bat <export-root> <version-string>
```

**Example:**
```cmd
release-cimtool.bat D:\CIMTool-Releases CIMTool-2.3.0-RC8
```
...
`release-cimtool.bat` invokes `install-jars.bat` during Step 3, which purges
