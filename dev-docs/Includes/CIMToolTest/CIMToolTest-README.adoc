...
CIMToolTest is structured as an Eclipse *plugin fragment* whose `MANIFEST.MF`
...
packages without requiring explicit exports. This is the standard Eclipse
pattern for test fragments: the tests run inside the same OSGi bundle as the
code under test, giving them access to package-private and internal APIs.
