...
with its classes shipping inside `cimtool.jar`. A self-hosted _Run As ▸ Eclipse
Application_ launch serves classes from `bin/` and can therefore mask whether a
class is actually packaged into the jar. Changes of this kind should be
validated from a product export, not only from a self-hosted launch.
